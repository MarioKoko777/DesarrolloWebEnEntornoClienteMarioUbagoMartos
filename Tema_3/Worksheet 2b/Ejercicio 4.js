...
/**
 * Devuelve aleatoriamente el nombre de un estudiante
 */
function alumnoAleatorio() {
  const indice = Math.floor(Math.random() * clase.length);
  const datos = clase[indice].split(", ");
  return datos[0]; // solo el nombre
}